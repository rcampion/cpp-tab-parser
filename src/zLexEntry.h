...
public:
	...
	int           getEntryId() const          { return m_nEntryId; }
	StatusCode    getName(char szText[] /* out */,int nBufferSize1 /* in */) const;
	ZLexEntry*    getNextEntryPtr() const     { return m_pNextEntry; }
	ZLexEntry*    getPrevEntryPtr() const     { return m_pPrevEntry; }
	StatusCode    getStatus() const		        { return m_eStatus;  }
	int           getListSize()	const         { return m_nListSize;}
	StatusCode    reInit();
	void		      setNextEntryPtr(ZLexEntry* pNext) { m_pNextEntry = pNext; }
	void		      setPrevEntryPtr(ZLexEntry* pPrev) { m_pPrevEntry = pPrev; }
